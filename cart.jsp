        ...
        <body>
            ...
            <div class="hello-xx-x1">
                <% // Database connection parameters for cart String cartJdbcUrl="jdbc:oracle:thin:@localhost:1521:xe" ;
                    ...
                    cartConnection=null; PreparedStatement cartStatement=null; try { // Load the JDBC driver for cart
                    Class.forName("oracle.jdbc.driver.OracleDriver"); // Establish the connection for cart
                    cartConnection=DriverManager.getConnection(cartJdbcUrl, cartUsername, cartPassword); // Prepare a
                    statement for inserting data into the cart table String
                    insertQuery="INSERT INTO cart (email, product_description, product_price, product_image_link) VALUES (?, ?, ?, ?)"
                    ; cartStatement=cartConnection.prepareStatement(insertQuery); // Retrieve user email and product
                    details from session String userEmail=(String) session.getAttribute("email"); // Use "email" as the
                    attribute name if (userEmail==null || userEmail.isEmpty()) { out.println("<p>User email not found in
                    session. Please login and try again.</p>");
                    } else {
                    ...
                    if (rowsAffected > 0) {
                    out.println("<u>
                        <p class='hello-hello-hello-hello'>Successfully added to cart!</p>
                    </u>");
