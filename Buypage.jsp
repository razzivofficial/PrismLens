    ...
    <%@ page contentType="text/html;charset=UTF-8" %>
        ...
        <body>
            ...
            <div class='product-container-top-top-top'>
                ...
                <% // Database connection parameters String jdbcUrl="jdbc:oracle:thin:@localhost:1521:xe" ; String
                    username="system" ; String password="admin" ; // JDBC connection Connection connection=null;
                    Statement statement=null; ResultSet resultSet=null; try { // Load the JDBC driver
                    Class.forName("oracle.jdbc.driver.OracleDriver"); // Establish the connection
                    connection=DriverManager.getConnection(jdbcUrl, username, password); // Create a statement
                    statement=connection.createStatement(); // Execute a query to fetch data from the products table
                    String query="SELECT * FROM products" ; resultSet=statement.executeQuery(query); if
                    (!resultSet.isBeforeFirst()) { System.out.println("No rows in the result set"); } else {
                    System.out.println("Rows in the result set"); } // Iterate through the result set and display
                    product information while (resultSet.next()) { String imageLink=resultSet.getString("image_link");
                    String description=resultSet.getString("description"); double price=resultSet.getDouble("price"); //
                    Display product information in a container with "Add to Cart" button out.println("<div
                    class='product-container-top-top'>");
                    ...
                    out.println("<div class='product-container'>");
                        ...
                        out.println("<form action='cart.jsp' method='post'>");
                            ...
                            out.println("<input id='xxee313' type='submit' name='addToCartButton'
                                value='Add to Cart'>");
                            out.println("</form>");
